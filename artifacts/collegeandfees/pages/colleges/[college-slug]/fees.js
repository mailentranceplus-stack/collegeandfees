...
function WaButton({ href, children, size = "md" }) {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" className={size === "sm" ? "btn-whatsapp-sm" : "btn-whatsapp"}>
...
export default function CollegeFeesPage({ college, fees, content, ranking, slug }) {
  ...
  return (
    <>
      ...
      {/* Hero Banner */}
      {COLLEGE_IMAGES[slug] && (
        <div style={{ overflow: "hidden", background: "var(--card)", maxHeight: "200px" }}>
          <img src={COLLEGE_IMAGES[slug]} alt={college.name} className="college-hero-banner" loading="eager" />
        </div>
      )}
